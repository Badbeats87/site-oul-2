...
class AnalyticsManager {
  ...
  renderOverviewStats() {
    ...
    let html =
      '<div class="analytics-section"><h3>Inventory Statistics</h3><div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">';

    if (data.totalInventory !== undefined) {
      html +=
        '<div class="stat-box"><label>Total Inventory</label><div class="value">' +
        data.totalInventory +
        '</div></div>';
    }
    ...
    if (data.byStatus) {
      html +=
        '<div class="stat-box"><label>Status Breakdown</label><div style="font-size: 12px;">';
    ...
    if (data.priceStats) {
      ...
      html +=
        '<div class="stat-box"><label>Price Stats</label><div style="font-size: 12px;"><strong>Avg:</strong> $' +
        parseFloat(stats.avg || 0).toFixed(2) +
        '<br><strong>Min:</strong> $' +
        parseFloat(stats.min || 0).toFixed(2) +
        '<br><strong>Max:</strong> $' +
        parseFloat(stats.max || 0).toFixed(2) +
        '</div></div>';
    }
  ...
  renderLowStockTable() {
    ...
    let html =
      '<div class="analytics-section"><h3>Low Stock Alerts</h3><table class="table"><thead><tr><th>Album</th><th>Artist</th><th>LIVE Count</th></tr></thead><tbody>';
  ...
  renderSalesMetrics() {
    ...
    let html =
      '<div class="analytics-section"><h3>Sales Metrics</h3><div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">';

    if (data.totalSold !== undefined) {
      html +=
        '<div class="stat-box"><label>Items Sold</label><div class="value">' +
        data.totalSold +
        '</div></div>';
    }
    ...
    if (data.totalRevenue !== undefined) {
      html +=
        '<div class="stat-box"><label>Total Revenue</label><div class="value">$' +
    ...
    if (data.averagePrice !== undefined) {
      html +=
        '<div class="stat-box"><label>Average Price</label><div class="value">$' +
        parseFloat(data.averagePrice || 0).toFixed(2) +
        '</div></div>';
    }
